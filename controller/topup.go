...
func GetTopUpInfo(c *gin.Context) {
	...
	// 如果启用了 USAD 支付，添加到支付方法列表
	enableUSAD := isUSADTopUpEnabled()
	if enableUSAD {
		hasUSAD := false
		for _, method := range payMethods {
			if method["type"] == model.PaymentMethodUSAD {
				hasUSAD = true
				break
			}
		}
		if !hasUSAD {
			payMethods = append(payMethods, map[string]string{
				"name":      "USAD",
				"type":      model.PaymentMethodUSAD,
				"color":     "rgba(var(--semi-teal-5), 1)",
				"min_topup": strconv.Itoa(setting.UsadMinTopUp),
			})
		}
	}
	...
	data := gin.H{
		...
		"usad_min_topup":          setting.UsadMinTopUp,
